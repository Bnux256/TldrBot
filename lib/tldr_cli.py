...
def is_in_cache(
    input_command: str, platform: str = "common", language: str = None
) -> bool:
    """
    ...
    if is_in_cache(input_command, platform, language):
        if language:
            directory_path = os.path.join(
                PAGES_DIR, ("pages." + language), platform, (input_command + ".md")
            )
        else:
            directory_path = os.path.join(
                PAGES_DIR,
                ("pages" + (str(language or ""))),
                platform,
                (input_command + ".md"),
            )
...
def get_languages() -> list[str]:
    ...
    dirs = [
        ...
        if os.path.isdir(os.path.join(PAGES_DIR, directory)) and directory != "pages"
    ]
