...
async def autocomp_langs(inter: disnake.ApplicationCommandInteraction, user_input: str):
    ...
    languages = sorted(
        [
            lang
            for lang in tldr_cli.get_languages()
            if lang.lower().startswith(user_input.lower())
        ]
    )
...
async def autocomp_platform(
    ...
    platforms = [
        platform
        for platform in tldr_cli.get_platforms()
        if platform.lower().startswith(user_input.lower())
    ]
    if len(platforms) > 25:
        return platforms[:24]
    else:
...
async def autocomp_command(
    ...
    commands = [
        command
        for command in tldr_cli.get_commands()
        if command.lower().startswith(user_input.lower())
    ]
    if len(commands) > 25:
        return commands[:24]
    else:
...
async def tldr(
    ...
    command: str = commands.Param(
        autocomplete=autocomp_command, description="Choose a command!"
    ),
    platform: str = commands.Param(
        default="common",
        autocomplete=autocomp_platform,
        description="optional - default: common - choose the platform you need",
    ),
    language: str = commands.Param(
        default="en",
        autocomplete=autocomp_langs,
        description="optional - default: english - choose the prefered language",
    ),
):
    logging.info(
        "User Entered: tldr %s (platform: %s; language: %s)"
        % (command, platform, (language or "en"))
    )
    ...
    if md is None:
        error_msg: str = (
            "Command doesn't exist in cache. To update, run `/update` command."
        )
        embed: disnake.Embed() = disnake.Embed(
            description=error_msg, colour=disnake.Colour.from_rgb(237, 66, 69)
        )
        embed.set_author(
            name="Error",
            icon_url="https://images-ext-2.discordapp.net/external/6HgbQ8ajjgJozMXg37BWe53K5YTN3YMVmWC93ioekY8/https/raw.githubusercontent.com/twitter/twemoji/master/assets/72x72/1f6ab.png",
        )
        ...
        for cur_platform in tldr_cli.get_platforms():
            if tldr_cli.is_in_cache(command, platform=cur_platform):
                embed.add_field(
                    name="This command exists in " + cur_platform,
                    value="You can run: `/tldr command:"
                    + command
                    + " platform: "
                    + cur_platform
                    + "`",
